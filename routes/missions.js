...
router.get("/", async (req, res) => {
  const allMissions = await Mission.find().populate('hostStructure').populate('missionReferant').populate('supportStructure').populate('coordinationStructure').populate('projectReferant')
  res.json(allMissions);
});

router.get("/:id", async (req, res) => {
  const mission = await Mission.findById(req.params.id).populate('hostStructure').populate('missionReferant').populate('supportStructure').populate('coordinationStructure').populate('projectReferant');
  res.json(mission);
});
...
router.post("/create", async (req, res, next) => {
  if (
    ...
    const { travel,
      ...
      sendingStructure,
      visa,
      wifi } = financialInformations
      
      const { missionReferantName,
        missionReferantSurname,
        missionReferantEmail,
        missionReferantPhone } = missionReferant
